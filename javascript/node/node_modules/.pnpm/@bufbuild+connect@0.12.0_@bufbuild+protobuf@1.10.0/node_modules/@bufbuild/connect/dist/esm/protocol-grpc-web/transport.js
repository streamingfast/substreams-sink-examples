        ...
        async unary(service, method, signal, timeoutMs, header, message) {
            ...
            return await runUnaryCall({
                ...
                next: async (req) => {
                    ...
                    const { compression } = validateResponseWithCompression(opt.acceptCompression, uRes.status, uRes.header);
                                ...
                                try {
                                    ...
                                    if (env.end) {
                                        ...
                                        if (message !== undefined) {
                                            throw new ConnectError("protocol error: received extra output message for unary method", Code.InvalidArgument);
                                        }
                                        message = env.value;
                                    }
                                }
                    ...
                    if (trailer === undefined) {
                        throw new ConnectError("protocol error: missing trailer", Code.InvalidArgument);
                    }
                    validateTrailer(trailer);
                    if (message === undefined) {
                        throw new ConnectError("protocol error: missing output message for unary method", Code.InvalidArgument);
                    }
                    return {
                        stream: false,
                        service,
            ...
            return runStreamingCall({
                ...
                next: async (req) => {
                    const uRes = await opt.httpClient({
                        ...
                        header: req.header,
                        signal: req.signal,
                        body: pipe(req.message, transformNormalizeMessage(method.I), transformSerializeEnvelope(serialization.getI(opt.useBinaryFormat)), transformCompressEnvelope(opt.sendCompression, opt.compressMinBytes), transformJoinEnvelopes(), { propagateDownStreamError: true }),
                    });
                    const { compression, foundStatus } = validateResponseWithCompression(opt.acceptCompression, uRes.status, uRes.header);
